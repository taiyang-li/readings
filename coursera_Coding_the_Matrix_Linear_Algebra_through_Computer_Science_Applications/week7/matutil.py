...
def efficient_rowdict2mat(rowdict):
    col_labels = value(rowdict).D                # get col labels
...
def identity(D, one):          # get the identity matrix S
...
def keys(d):
    ...
    """
    return d.keys() if isinstance(d, dict) else range(len(d))  # get all the index 

def value(d):  
    """Given either a dict or a list, returns one of the values. 
       Intended for coldict2mat and rowdict2mat.
    """ 
    return next(iter(d.values())) if isinstance(d, dict) else d[0]   # get the first element

def mat2rowdict(A): # transfer a mat to a row dict 
...
def mat2coldict(A):  # transfer a mat to col dict (whose element is a column of A)
...
def coldict2mat(coldict):  # transfer a dict or list (whose element is vector, and represents a column of mat) to a mat
    """
...
def rowdict2mat(rowdict):   # transfer list of vectors to mat 
    """
    Given a dictionary or list whose values are Vecs, returns the Mat having these
...
def listlist2mat(L):     # transfer list of list to mat
...
def submatrix(M, rows, cols):  #get submatrix according to rows and cols 
